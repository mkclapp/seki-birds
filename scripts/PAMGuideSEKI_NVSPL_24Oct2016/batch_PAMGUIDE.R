...
#NOTE: all plotting options are turned off because of errors with the code.
...
#___________________________________________________________________________________________________
#WORKED FOR:
#DETOGRASSCON__1__0_%Y%m%d_%H%M%S_000.wav

#___________________________________________________________________________________________________
#CALIBRATE
# SM2 = mh=-35, G=xx, vADC=1.414
# SM3 = mh=-36, G=24+XX, vADC=1

#BEFORE RUNNING....
#1) check the calibration settings
#2) check the timestring
...
workingDir <-"G:\\NPS_DATA\\SEKI_MClapp\\test"
site = "EASTLA1"
...
#calibrated
PAMGuide(chunksize = 500, atype = 'TOL', timestring = "EASTLA1_%Y%m%d_%H%M%S.wav",
         r=0, outwrite=1, plottype = "None", calib=1, envi="Air", ctype="TS", Mh=-35, G=24, vADC=1.414)
...
Meta(chunksize = 500, atype = 'TOL', timestring = "EASTLA1_%Y%m%d_%H%M%S.wav",
         r=0, outwrite=1, plottype = "None", calib=1, envi="Air", ctype="TS", Mh=-35, G=24, vADC=1.414)
...
tempOutput <- cbind(site, tString, 0, 0, 0, round(a, 1), matrix(rep(NA,dim(a)[1] * nBlankCols), nrow=dim(a)[1], ncol=nBlankCols))
tempOutput[,36] = dBA
