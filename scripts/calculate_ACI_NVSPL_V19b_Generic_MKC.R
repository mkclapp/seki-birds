...
# v16b- option to not do day cluster because too big a file!!
# v16c- directories set up and NO caluclations per day
# v17- fixed error with inf in the matrix- just removed and then skip because too short for AI calculations
#     and added function to start on specific day, set in Options
# v18- added a new calculation for ACI with the intensity values instead of SPL to correct for erronious values when we get negativee SPLs
# v19- fixed code to deal with INF values in NVSPL files- remove those rows!
# v19b- removed the movefiles code to prevent accidental loss of data
...
## -------------------------------------------------------------------------
## LOAD LIBRARIES
library(tuneR) # manipulate and analyze wav files
library(seewave) # "provides functions for analysing, manipulating, displaying, editing and synthesizing time waves"
library(ineq) #"computes the inequality within a vector according to the specified inequality measure"
library(fftw)
library(soundecology)
library(vegan)
# library(RAtmosphere) # apparently DNE?
library(e1071)  
library(moments)
library(RSNNS)
#library(sparcl) # apparently DNE?
library(dbscan)
library(cluster)
library(factoextra)
library(NbClust)
...
## -------------------------------------------------------------------------
## GET FILES
workingDir = "../../data/NVSPL" #"E:\\NPS_DATA\\BioacousticPhenology\\ALL\\NVSPL_copy\\toProcess"
...
Outdir = "../../data/indices/2018_09_27" #E:\\NPS_DATA\\BioacousticPhenology\\ALL\\ANALYSIS\\AcousticIndex"
...
#how many bands between 1.6kHz and 8 kHz (1.6, 2, 2.5, 3.15, 4, 5, 6.3, 8)
fbinMax    = 8 #sets the frequency bins to looks across for cluster analysis- relates to frequency bins in biological band- 
...
#what day to start on- option if lots of files in directory or error!
#startday =  user <- dlgInput("What is the NVSPL start day (year_month_day) or enter 1", Sys.info()["user"])$res
#startday =  user <- dlgInput("What is the NVSPL start day (year_month_day)")$res
startday = 1

#FLAGS
savFile   = 1 #saves 1 files- timestep values
...
for (ss in 3:length(mySites)  )  #1:length(mySites) START loop through sites 
{
  ss=3
  OUTPUT_sites = NULL
  OUTPUT_sitesD = NULL
  ...
  if (startday == 1) { startday = uDayIndex[1]}
  dindx = grep(startday, uDayIndex)
  
  for ( dd in dindx:length(uDayIndex)) # 1:length(uDayIndex) START loop through unique site days 
  {
    # dd = 1   #for testing
    ...
    for(i in 0:23) # possible hours unique( dayMatrix$Hr )
    {
      for(j in 1:(60/Timestep)) # Timestep for each hour : unique( dayMatrix$Min )
      {
        ...
        if( dim(workingMatrix)[1] >= (Timestep * 60)- 60 )
        {
          ...
          Output = rbind(Output, cbind(mySites[ss], siteDays[dd], 
                                       ...
                                       pk, pkd, pks, Hm, HvPres, HvSPL, dif_L10L90, Mamp, NumCL, SP2,CLdets, version))
  ...
  if (savFile == 1) {
    # TIMESTEP DATA
    outFileName <- paste0(Outdir, "/", mySites[ss], "_",Timestep, "mins", "_NVSPL_AcousticIndex_Created", dstmp, ".csv")
